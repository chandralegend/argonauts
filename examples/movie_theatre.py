...
from argonauts import interactive, LogBook
...
@interactive(logbook=args, process_name="Booking your movie experience! Please wait...")
def select_movie(
    movie_title: Movie = Movie.THE_MATRIX,
    num_tickets: int = 1,
    imax: bool = False
) -> list[str]:
...
@interactive(logbook=args, process_name="Making your snacks! Stop drooling!", include_params=["snacks", "extra_butter"])
def select_snacks(
    ticket_ids: list[str],
    snacks: list[Snack],
    extra_butter: bool = False
):
    ...
    if args.imax:
        print(f"Congratulations! As an IMAX ticket holder, you get a {args.num_tickets} free soda!")
